...
interface CustomLayer {
    ...
    elevationNXBuffer: WebGLBuffer | null; // Stores elevation data for each vertex
    elevationNZBuffer: WebGLBuffer | null; // Stores elevation data for each vertex
    onAdd: (map: mapboxgl.Map, gl: WebGLRenderingContext) => void;
    render: (gl: WebGLRenderingContext, matrix: number[]) => void;
}

// Define a custom WebGL layer that will be added to the map for data visualization
export const visualizationLayer: CustomLayer = {
    id: 'visualization',
    type: 'custom',
    aElevation: 0,
    aElevationNX: 0,
        ...
        for (let i = 0; i < gridWidth; i++) {
            for (let j = 0; j < gridHeight; j++) {
                ...
                for (let k = 0; k < 6; k++) {  // 6 vertices per square (2 triangles)
                    ...
                    if (i == 10 && j == 10) {
                        console.log(`elevations ${elevations[elevations.length - 1]} elevationsNX ${elevationsNX[elevationsNX.length - 1]} elevationsNZ ${elevationsNZ[elevationsNZ.length - 1]}`);
                    }
                }
            }
        }
